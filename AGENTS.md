...
- `setup-worktree-workspace` transactionally converts one eligible regular checkout into the supported worktree-oriented container. It creates `.smallpowers` metadata and rich container instructions but no default branch-prefix directories or prefix allowlist. It must preview and revalidate the exact topology, preserve repository state, classify relevant remotes as GitHub or GitLab when possible, and separately report the matching CLI's installation, version, and authentication plus any matching skill advertised in the active task. The generated instructions cover both `gh` and `glab`. Setup must not install, authenticate, or invoke either integration; missing forge automation alone does not block the local conversion.
...
- `work-in` accepts one relative multi-component branch path plus optional `--ref`, resolves it below the current container, and reuses or creates that branch-mirrored worktree. A new branch defaults to the canonical checkout's current `HEAD`; an existing branch is never reset to `--ref`. The skill injects a concise task-local binding prompt, and every later mutation must revalidate repository identity, worktree path, and branch.
- `cleanup-worktree` previews removal of one exactly resolved, merged non-primary worktree and its eligible local branch, then updates the canonical primary worktree's base branch by fast-forward only after exact state-bound confirmation. It may directly delete content only from validated generated `.venv` and `target` directories named in that preview; those deletions are unrecoverable and must be reported. After core cleanup, it may non-recursively prune only previewed, unchanged empty branch-path ancestors and must stop before the workspace container. It must not delete a remote branch.
